...
static async discardSelectedCard() {
  // Get the selected card ID
  const selectedCardId = UIManager.selectedCard;
  if (!selectedCardId) {
    ui.notifications.warn('No card selected');
    return false;
  }
  ...
  if (!piles.hand.cards.has(selectedCardId)) {
    ui.notifications.warn('Selected card no longer in hand');
    UIManager.clearCardSelection();
    return false;
  }
  ...
  try {
    ...
    await piles.hand.pass(piles.discard, [selectedCardId], { chatNotification: false });
